...
public class EntityPassengerRheingold extends EntityRollingStock implements IPassenger {
	...
	public void updateRiderPosition() {
		...
		*/
		...
		double rads = this.rotationYawClientReal-90 * 3.141592653589793D / 180.0D;
		if (pitchRads == 0.0) {
			riddenByEntity.setPosition(bogieX1- (Math.cos(rads)*-0.2), pitch1+ (Math.tan(this.anglePitchClient * 3.141592653589793D / 180.0D)*0.2), bogieZ1- (Math.sin(rads)*-0.2));
		}
		else if (pitchRads > -1.01 && pitchRads < 1.01) {
			riddenByEntity.setPosition(bogieX1- (Math.cos(rads)*-0.2), pitch+ (Math.tan(this.anglePitchClient * 3.141592653589793D / 180.0D)*0.2), bogieZ1- (Math.sin(rads)*-0.2));
		}
	}
