...
public class BlockHayesBumper extends BlockContainer {

    private IIcon texture;

    public BlockHayesBumper() {
        super(Material.iron);
    ...
    public void onBlockPlacedBy(World world, int par2, int par3, int par4, EntityLivingBase living, ItemStack stack) {
        TileHayesBumper te = (TileHayesBumper) world.getTileEntity(par2, par3, par4);
    ...
    public TileEntity createNewTileEntity(World world, int meta) {
        return new TileHayesBumper(meta);
    }

    @Override
    @SideOnly(Side.CLIENT)
    public void registerBlockIcons(IIconRegister iconRegister) {
        texture = iconRegister.registerIcon(Info.modID.toLowerCase() + ":HayesBumper");
    }
}