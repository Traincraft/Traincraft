...
public class LinkHandler {
	...
	public void handleStake(EntityRollingStock entityOne, AxisAlignedBB customBoundingBox) {
		if(entityOne.isAttaching){
			...
			if(entityOne.bogieLoco != null){
				entities.addAll(worldObj.getEntitiesWithinAABBExcludingEntity(entityOne, entityOne.bogieLoco.getEntityBoundingBox().expand(7, 5, 7)));
			}
			...
			if(cart1.bogieLoco != null || cart2.bogieLoco != null){
				...
				if(cart1.bogieLoco != null && cart2.bogieLoco == null){
					...
					for(int i = 0; i < distancesX.length; i++){
						euclidian[i] = MathHelper.sqrt((distancesX[i] * distancesX[i]) + (distancesZ[i] * distancesZ[i]));
					}
	...
	private boolean canCartBeAdjustedBy(Entity cart1, Entity cart2) {
		...
		return true;//!RailTools.isCartLockedDown((EntityMinecart) cart1);
	}
	...
	private void StakePhysic(EntityRollingStock cart1, EntityRollingStock cart2, int linkIndex) {
		...
		if(cart2.isAttached && cart1.isAttached && areLinked(cart2, cart1)){
			...
			if(d2 > 20){
				...
				if(cart1.world != null){
					EntityPlayer player = cart1.world.getClosestPlayer(cart1.posX, cart1.posY, cart1.posZ, 300, false);
					if(player != null){
						player.sendMessage(new TextComponentString(String.format("[TRAINCRAFT] The rolling stock at %d %d %d had a problem loading and has lost its link. Attached cart was too far away", (int) cart1.posX, (int) cart1.posY, (int) cart1.posZ)));
					}
				}
			...
			double vecNorm = MathHelper.sqrt(vecX * vecX + vecZ * vecZ);
