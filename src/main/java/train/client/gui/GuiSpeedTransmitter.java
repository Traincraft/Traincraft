...
public class GuiSpeedTransmitter extends GuiScreen {
    ...
    public void updateScreen() {
        ...
        if (speedLimitTextField.isFocused()) {
            speedLimitTextField.updateCursorCounter();
        }

        if (nextSpeedLimitTextField.isFocused()) {
            nextSpeedLimitTextField.updateCursorCounter();
        }

        if (nextSpeedXTextField.isFocused()) {
            nextSpeedXTextField.updateCursorCounter();
        }

        if (nextSpeedYTextField.isFocused()) {
            nextSpeedYTextField.updateCursorCounter();
        }

        if (nextSpeedZTextField.isFocused()) {
            nextSpeedZTextField.updateCursorCounter();
        }
    }
    ...
    protected void keyTyped(char par1, int par2) {
        if (Character.isDigit(par1) || par1 == KeyEvent.VK_BACK_SPACE || par1 == KeyEvent.VK_PERIOD) {
            if (speedLimitTextField.isFocused()) {
                speedLimitTextField.textboxKeyTyped(par1, par2);
            }

            if (nextSpeedLimitTextField.isFocused()) {
                nextSpeedLimitTextField.textboxKeyTyped(par1, par2);
            }

            if (nextSpeedXTextField.isFocused()) {
                nextSpeedXTextField.textboxKeyTyped(par1, par2);
            }

            if (nextSpeedYTextField.isFocused()) {
                nextSpeedYTextField.textboxKeyTyped(par1, par2);
            }

            if (nextSpeedZTextField.isFocused()) {
                nextSpeedZTextField.textboxKeyTyped(par1, par2);
            }
        }
    ...
    public static boolean isInteger(String s) {
        ...
        // We can only get here if we do not return false.
        return true;
    }
