...
package railcraft.api.electricity;

import java.util.EnumSet;
import java.util.HashSet;
import java.util.Map;
import java.util.Set;
import railcraft.api.core.WorldCoordinate;
import railcraft.api.tracks.ITrackInstance;
import railcraft.api.tracks.ITrackTile;
    ...
    public static Set<IElectricGrid> getMutuallyConnectedObjects(IElectricGrid gridObject) {
        ...
        for (Map.Entry<WorldCoordinate, EnumSet<IElectricGrid.ChargeHandler.ConnectType>> position : gridObject.getChargeHandler().getPossibleConnectionLocations().entrySet()) {
            ...
            if (otherObj != null && position.getValue().contains(otherObj.getChargeHandler().getType())) {
                EnumSet<IElectricGrid.ChargeHandler.ConnectType> otherType = otherObj.getChargeHandler().getPossibleConnectionLocations().get(myPos);
